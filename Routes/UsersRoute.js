  ...
  if (!token) {
    ...
    await jwt.verify(token, "secret-key", (err, decoded) => {
      if (err) {
        ...
        //update user next
        const sql =
          "UPDATE delivery SET `DfirstName`=?, `DlastName`=?, `DphoneNumber`=?, `Dstreet`=?, `Ddirections`=?, `Dcity`=?, `Dstate`=?, `Dlocalgva`=? WHERE customer_id = ?";
  ...
  if (!token) {
    ...
    jwt.verify(token, "secret-key", (err, decoded) => {
      if (err) {
        ...
        const sql =
          "SELECT	DeliveryID , DfirstName, DlastName, DphoneNumber, Dstreet, Ddirections, Dcity, Dstate, Dlocalgva  FROM delivery WHERE customer_id = ?"
